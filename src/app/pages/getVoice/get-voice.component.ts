...
import { SnackBarComponent } from 'src/app/components/snackbar.component';
...
export class GetVoiceComponent {
  isLoading = false; // Spinner visibility control
  currentWord: string = '';
      ...
      if (result === 'yes') {
        this.api
          ...
          .subscribe(
            ...
            (error: any) => {
              this.openSnackBar(
                'Ekki tókst að eyða með skilaboðum => ' + error.message
              );
            }
          );
      }
    ...
    for (const element of this.wordsplit) {
      if (element[0] === '<') {
        if (element[1] + element[2] === 'ph') {
          let word = element.substring(
            element.indexOf('>') + 1,
            element.indexOf('</')
          );
          let wordWithT = `${word.substring(
            0,
            word.indexOf('ll')
          )}tl${word.substring(word.indexOf('ll') + 2, word.length)}`;
          let wordWithoutT = `${word.substring(
            0,
            word.indexOf('ll')
          )}ll${word.substring(word.indexOf('ll') + 2, word.length)}`;
          let newStringArr = [word, sampa(wordWithT), sampa(wordWithoutT)];
